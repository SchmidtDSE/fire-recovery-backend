...
class TestIndexRegistry:
    ...
    def test_add_calculator_duplicate_raises_error(
        ...
        with pytest.raises(
            ValueError, match="Calculator for index 'nbr' already exists"
        ):
            registry.add_calculator(mock_calc)
    ...
    def test_replace_calculator_nonexistent_raises_error(
        self, registry: IndexRegistry
    ) -> None:
        ...
        with pytest.raises(
            ValueError, match="Calculator for index 'nonexistent' doesn't exist"
        ):
            registry.replace_calculator("nonexistent", mock_calc)

    def test_replace_calculator_name_mismatch_raises_error(
        self, registry: IndexRegistry
    ) -> None:
        ...
        with pytest.raises(
            ValueError,
            match="Calculator index name 'different_name' doesn't match requested 'nbr'",
        ):
            registry.replace_calculator("nbr", mock_calc)
    ...
    def test_get_calculators_requiring_both_periods(
        self, registry: IndexRegistry
    ) -> None:
    ...
    def test_validate_dependencies_with_custom_calculator(
        self, registry: IndexRegistry
    ) -> None:
    ...
    def test_logging_on_setup(self, mock_logger, registry: IndexRegistry) -> None:
        ...
        setup_log = next(
            (msg for msg in call_args if "Setting up index calculators" in msg), None
        )
        ...
        registered_log = next(
            (
                ...
                if "Registered" in msg and "index calculators" in msg
            ),
            None,
        )
    ...
    def test_calculator_instances_are_shared_correctly(
        self, registry: IndexRegistry
    ) -> None:

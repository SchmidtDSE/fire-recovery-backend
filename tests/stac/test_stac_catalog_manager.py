...
class TestSTACCatalogManager:
    ...
    @pytest.fixture
    async def catalog_manager(
        self, minio_storage: StorageInterface
    ) -> AsyncGenerator[STACCatalogManager, None]:
        """Create a STACCatalogManager instance for testing"""
        manager = STACCatalogManager(base_url=STAC_TEST_BASE_URL, storage=minio_storage)
        yield manager
        # Cleanup after test
    ...
    async def test_add_veg_matrix_item(
        ...
        collection_items = await catalog_manager.get_collection_items(
            "vegetation-matrices"
        )
        assert len(collection_items) == 1

    @pytest.mark.asyncio
    async def test_get_catalog_and_collections(
        self, catalog_manager: STACCatalogManager
    ) -> None:

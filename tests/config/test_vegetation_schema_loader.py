...
from src.config.vegetation_schema_loader import (
...
class TestVegetationSchemaConfig:
    ...
    def test_load_from_file_not_found(self):
        ...
        assert "Configuration file non_existent_file.json not found" in str(
            exc_info.value
        )
...
class TestVegetationSchemaLoader:
    ...
    def test_get_schema_not_found(self, temp_config_file):
        ...
        assert "No vegetation schema found for park unit 'NONEXISTENT'" in str(
            exc_info.value
        )
        ...
        with patch.object(loader, "_load_schemas") as mock_load:
            mock_load.return_value = {
                "JOTR": VegetationSchema(
                    vegetation_type_field="test", geometry_column="geom"
                )
            }

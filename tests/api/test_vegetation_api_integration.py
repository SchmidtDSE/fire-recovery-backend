...
class TestVegetationAPIIntegration:
    ...
    def test_vegetation_resolve_api_with_park_unit_id(
        ...
        with patch(
            "src.routers.fire_recovery.execute_vegetation_resolution_command"
        ) as mock_execute:
    ...
    def test_vegetation_resolve_api_without_park_unit_id(
        self, client, sample_request_data
    ):
    ...
    def test_vegetation_resolve_api_with_invalid_park_unit_id(
        self, client, sample_request_data
    ):
    ...
    def test_vegetation_resolve_api_request_validation(self, client):
        ...
        response = client.post(
            "/fire-recovery/process/resolve_against_veg_map", json=invalid_request
        )
    ...
    def test_vegetation_resolve_api_with_optional_fields(
        self, client, sample_request_data
    ):
...
class TestVegetationSchemaAPIIntegration:
    ...
    def test_api_error_handling_with_schema_failures(self, client):
        ...
        with patch(
            ...
            response = client.post(
                "/fire-recovery/process/resolve_against_veg_map", json=request_data
            )
    ...
    def test_api_schema_parameter_types(self, client):
        ...
        with patch("src.routers.fire_recovery.execute_vegetation_resolution_command"):
            response = client.post(
                "/fire-recovery/process/resolve_against_veg_map", json=request_with_null
            )
            assert response.status_code == 200
        ...
        response = client.post(
            "/fire-recovery/process/resolve_against_veg_map", json=request_with_invalid
        )
        assert response.status_code == 422  # Validation error

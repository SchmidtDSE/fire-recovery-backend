...
class TestStatisticalWarningsFix:
    ...
    def test_create_veg_json_structure_nan_handling(self):
        ...
        df_with_nans = pd.DataFrame(
            {
                ...
                "unburned_percent": [
                    60.0,
                    0.0,
                ],  # Already processed by add_percentage_columns
        ...
        normal_community = next(
            c for c in result["vegetation_communities"] if c["name"] == "Normal"
        )
        assert normal_community["total_hectares"] == 100.0
        assert (
            normal_community["percent_of_park"] == 100.0
        )  # Normal is 100% of total park

        # The NaN community should be handled gracefully
        nan_community = next(
            c for c in result["vegetation_communities"] if c["name"] == "NaN_Type"
        )
    ...
    def test_no_warnings_during_percentage_calculations(self):
        ...
        with warnings.catch_warnings(record=True) as warning_list:
            ...
            assert len(division_warnings) == 0, (
                f"Division warnings: {[str(w.message) for w in division_warnings]}"
            )
            ...
            with warnings.catch_warnings(record=True) as warning_list:
                ...
                stat_warnings = [
                    ...
                    if any(
                        ...
                        for phrase in ["degrees of freedom", "invalid value", "divide"]
                    )
                ]

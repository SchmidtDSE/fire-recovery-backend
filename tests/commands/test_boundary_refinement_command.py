...
def mock_stac_manager() -> Mock:
    ...
    manager.create_fire_severity_item = AsyncMock(return_value="mock://stac/severity.json")
...
class TestBoundaryRefinementCommand:
    ...
    def test_get_required_permissions(self):
        ...
        command = BoundaryRefinementCommand()
        expected_permissions = ["stac:read", "stac:write", "storage:write", "computation:execute"]
    ...
    def test_validate_context_missing_fire_event_name(self, valid_context: CommandContext):
    ...
    def test_validate_context_invalid_geometry_type(self, valid_context: CommandContext):
        """Test context validation with invalid geometry type"""
        command = BoundaryRefinementCommand()
        valid_context.geometry = {"type": "Point", "coordinates": [0, 0]}  # Invalid type
        
        is_valid, error_msg = command.validate_context(valid_context)
        assert is_valid is False
        assert "geometry must be a Polygon or Feature object" in error_msg
    ...
    async def test_execute_success_single_refinement(
        ...
        mock_process_geojson.return_value = (
            "mock://boundary.geojson",
            {
                ...
                "features": [{
                    "type": "Feature", 
                    "geometry": {"type": "Polygon", "coordinates": [[[-120, 35], [-119, 35], [-119, 36], [-120, 36], [-120, 35]]]},
                    "properties": {}
                }]
            },
    ...
    async def test_execute_failure_no_original_stac_item(
        ...
        valid_context.stac_manager.get_item_by_id.return_value = None  # No STAC item found
    ...
    async def test_execute_partial_success_stac_metadata_failure(
        ...
        valid_context.stac_manager.get_item_by_id.return_value = sample_stac_item
        valid_context.stac_manager.create_fire_severity_item.side_effect = Exception("STAC creation failed")
        ...
        # Verify partial success
        assert result.status == CommandStatus.PARTIAL_SUCCESS
        assert "Assets created but STAC metadata creation failed" in result.error_message
        assert result.has_assets()  # Assets should still be created

    @pytest.mark.asyncio
    async def test_execute_context_validation_failure(self, valid_context: CommandContext):
        """Test execution failure due to context validation"""
        command = BoundaryRefinementCommand()
        original_job_id = valid_context.job_id
        valid_context.geometry = None  # Make context invalid but keep job_id for CommandResult
    ...
    def test_feature_geometry_validation(self, sample_feature: Feature, mock_storage: Mock, 
                                       mock_storage_factory: Mock, mock_stac_manager: Mock, 
                                       mock_index_registry: Mock):
    ...
    async def test_iterative_refinement_workflow(
        ...
        mock_process_geojson.return_value = (
            "mock://refined_boundary_v2.geojson",  # Different URL but same storage path pattern
            {
                ...
                "features": [{
                    "type": "Feature",
                    "geometry": {"type": "Polygon", "coordinates": [[[-120, 35.1], [-119.1, 35.1], [-119.1, 36.1], [-120, 36.1], [-120, 35.1]]]},
                    "properties": {}
                }]
            },
        ...
        original_calls = [call for call in call_args_list if any(
        ...
        assert len([call for call in refined_calls if "output_filename" in str(call)]) == 6
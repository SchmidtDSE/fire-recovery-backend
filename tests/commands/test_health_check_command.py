...
def mock_index_registry() -> MagicMock:
    ...
    mock_registry.get_available_calculators.return_value = ["nbr", "dnbr", "rdnbr", "rbr"]
...
class TestHealthCheckCommand:
    ...
    def test_validate_context_success(self, health_command_context: CommandContext) -> None:
        """Test successful context validation"""
        command = HealthCheckCommand()
        assert command.validate_context(health_command_context) is True
    
    def test_validate_context_missing_job_id(self, health_command_context: CommandContext) -> None:
    ...
    async def test_execute_all_healthy(
        self, 
        health_command_context: CommandContext,
        mock_index_registry: MagicMock
    ) -> None:
    ...
    async def test_execute_with_unhealthy_component(
        ...
        mock_index_registry.get_available_calculators.side_effect = Exception("Registry failed")
    ...
    async def test_execute_with_exception_in_health_check(
        ...
        health_command_context.storage = None  # This should trigger not_configured, not an exception

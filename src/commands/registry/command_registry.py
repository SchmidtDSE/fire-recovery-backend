...
class CommandRegistry:
    ...
    def _setup_commands(self) -> None:
        ...
        try:
            from src.commands.impl.boundary_refinement_command import (
                BoundaryRefinementCommand,
            )
        ...
        try:
            from src.commands.impl.vegetation_analysis_command import (
                VegetationAnalysisCommand,
            )
    ...
    def _validate_registry(self) -> None:
        ...
        for command_name, command_class in self._command_classes.items():
            try:
                ...
                logger.debug(
                    f"Command '{command_name}' dependencies satisfied: {dependencies}"
                )
    ...
    def create_command(self, command_name: str, context: CommandContext) -> Command:
        ...
        if not command.validate_context(context):
            raise ValueError(f"Context validation failed for command '{command_name}'")

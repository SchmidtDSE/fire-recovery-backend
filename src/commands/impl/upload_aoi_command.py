...
class UploadAOICommand(Command):
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            execution_time = (time.time() - start_time) * 1000
            logger.error(
                f"AOI upload failed for job {context.job_id}: {str(e)}",
                exc_info=True
            )
    ...
    async def _process_shapefile_upload(
        ...
        try:
            ...
            zip_blob_name = f"{context.fire_event_name}/{context.job_id}/original_shapefile.zip"
    ...
    async def _create_boundary_stac_item(
        ...
        try:
            ...
            stac_item_url = f"stac://{context.fire_event_name}-boundary-{context.job_id}"

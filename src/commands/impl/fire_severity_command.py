...
class FireSeverityAnalysisCommand(Command):
    ...
    def validate_context(self, context: CommandContext) -> bool:
        ...
        if not prefire_dates or not postfire_dates:
            logger.error("prefire_date_range and postfire_date_range are required in computation_config")
            return False
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            indices = context.get_computation_config("indices", ["nbr", "dnbr", "rdnbr", "rbr"])
            ...
            stac_data = await self._fetch_satellite_data(
                context, prefire_date_range, postfire_date_range, collection, buffer_meters
            )
            ...
            asset_urls = await self._save_results_as_cogs(
    ...
    async def _fetch_satellite_data(
        ...
        try:
            ...
            if not items:
                raise ValueError(f"No satellite data found for date range {full_date_range}")
            ...
            logger.info(f"Using bands - NIR: {nir_band}, SWIR: {swir_band}, EPSG: {epsg_code}")
            ...
            prefire_data = self._subset_data_by_date_range(stacked_data, prefire_date_range)
            postfire_data = self._subset_data_by_date_range(stacked_data, postfire_date_range)
        ...
        try:
            ...
            for index_name in requested_indices:
                ...
                if calculator is None:
                    logger.warning(f"Calculator for index '{index_name}' not found, skipping")
                    continue
    ...
    async def _save_results_as_cogs(
        self,
        context: CommandContext,
        index_results: Dict[str, xr.DataArray]
    ) -> Dict[str, str]:
    ...
    def _get_buffered_bounds(self, geometry: Union[Polygon, Dict[str, Any]], buffer_meters: float) -> tuple:

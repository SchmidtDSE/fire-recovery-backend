...
class HealthCheckCommand(Command):
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            logger.info(
                f"Health check completed for job {context.job_id} in {execution_time:.2f}ms"
            )
            ...
            execution_time = (time.time() - start_time) * 1000
            logger.error(
                f"Health check failed for job {context.job_id}: {str(e)}", exc_info=True
            )
    ...
    async def _check_system_health(self, context: CommandContext) -> dict:
        ...
        if context.stac_manager:
            try:
                ...
                checks["stac_manager"] = {"status": "unhealthy", "error": str(e)}
        else:
        ...
        if context.index_registry:
            try:
                ...
                checks["index_registry"] = {"status": "unhealthy", "error": str(e)}
        else:

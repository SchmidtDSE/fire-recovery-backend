...
class BoundaryRefinementCommand(Command):
    ...
    def validate_context(self, context: CommandContext) -> Tuple[bool, str]:
        ...
        if not context.geometry:
            return False, "refined boundary geometry is required for boundary refinement"
    ...
    async def execute(self, context: CommandContext) -> CommandResult:
        ...
        try:
            ...
            logger.info(f"Processing {len(original_cog_item['assets'])} fire severity metrics")
            ...
            for metric, cog_data in original_cog_item["assets"].items():
                try:
                    ...
                    logger.info(f"Processed refined COG for metric {metric}: {refined_cog_url}")
                    
                except Exception as e:
                    logger.error(f"COG processing failed for metric {metric}: {str(e)}", exc_info=True)
            ...
            logger.info(f"Boundary refinement completed successfully in {execution_time_ms:.2f}ms")
            ...
            logger.error(f"Boundary refinement execution failed: {str(e)}", exc_info=True)
            execution_time_ms = (time.time() - start_time) * 1000
            return CommandResult.failure(
                job_id=context.job_id,
                fire_event_name=context.fire_event_name,

...
BASE_URL = os.environ.get(
    "STAC_BASE_URL", "https://storage.googleapis.com/national_park_service/stac"
)
...
async def get_stac_root():
    ...
    root = {
        "stac_version": "1.0.0",
        "stac_extensions": [
            "https://stac-extensions.github.io/collection/v1.0.0/schema.json"
        ],
        "description": "STAC API for Fire Recovery Analysis Products",
        "links": [
            {"rel": "self", "href": f"{BASE_URL}/stac", "type": "application/json"},
            {
                "rel": "root",
...
async def get_collection(fire_event_name: str):
    ...
    if not os.path.exists(parquet_path):
        raise HTTPException(
            status_code=404, detail=f"Collection not found: {fire_event_name}"
        )
    ...
    collection = {
        ...
        "extent": {
            "spatial": {
                "bbox": [
                    [-180, -90, 180, 90]
                ]  # This should be updated with actual bounds
            },
            "temporal": {
...
async def get_items(
    ...
    datetime: Optional[str] = Query(
        None, description="Datetime range (e.g. 2023-01-01/2023-12-31)"
    ),
):
    """
...
@router.get(
    "/collections/{fire_event_name}/items/{item_id}", response_model=Dict[str, Any]
)

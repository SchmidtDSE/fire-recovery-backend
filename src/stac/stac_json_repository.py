...
class STACJSONRepository:
    ...
    async def add_item(
        self, item_dict: Dict[str, Any], skip_validation: bool = False
    ) -> str:
        """
        ...
        return await self.storage.save_json(
            self.item_to_dict(item, skip_validation), file_path
        )
            ...
            if file_path.endswith(".json"):
                try:
                    ...
                    if (
                        ...
                        and properties.get("boundary_type") == boundary_type
                    ):
                        return item_data
                except Exception:
                    continue
